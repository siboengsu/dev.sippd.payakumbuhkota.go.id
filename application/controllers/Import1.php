            ...
            if(isset($_FILES["file"]["name"]))
            {
                ...
                foreach($object->getWorksheetIterator() as $worksheet)
                {
                    ...
                    for($row=2; $row<=$highestRow; $row++)
                    {
                        ...
                        $TARGETSUBKEGSEBELUM    = $worksheet->getCellByColumnAndRow(11, $row)->getValue();
                        $TARGETSUBKEG           = $worksheet->getCellByColumnAndRow(12, $row)->getValue();
                        $PAGUTDPA               = $worksheet->getCellByColumnAndRow(17, $row)->getValue();
                        $PAGUTIF                = $worksheet->getCellByColumnAndRow(18, $row)->getValue();
                        $PAGUTPLUS              = $worksheet->getCellByColumnAndRow(19, $row)->getValue();
                        $SUBKEGRKPDKEY          = $worksheet->getCellByColumnAndRow(20, $row)->getValue();
                        $KEGRKPDKEY             = $worksheet->getCellByColumnAndRow(21, $row)->getValue();
                        $PGRMRKPDKEY            = $worksheet->getCellByColumnAndRow(22, $row)->getValue();
                        $UNITKEY                = $worksheet->getCellByColumnAndRow(23, $row)->getValue();
                        $JENIS                  = $worksheet->getCellByColumnAndRow(24, $row)->getValue();
                        if($JENIS == 'PRO')
                        {
                            $TOLOKUR            = $worksheet->getCellByColumnAndRow(13, $row)->getValue();
                            $TARGET             = $worksheet->getCellByColumnAndRow(15, $row)->getValue();
                            $TARGETSEBELUMP      = $worksheet->getCellByColumnAndRow(14, $row)->getValue();         
                        }
                        if($JENIS == 'PRO')
                        {
							$tahun = $this->KDTAHUN;
                            $cekprog = $this->db->query("SELECT PGRMRKPDKEY FROM PGRRKPD WHERE PGRMRKPDKEY = '$PGRMRKPDKEY' AND UNITKEY = '$UNITKEY' AND KDTAHUN = '$tahun'")->row_array();
                            if(count($cekprog) > 0)
                            {
                        ...
                        if($JENIS == 'KEG')
                        {
                            $cekprog = $this->db->query("SELECT KEGRKPDKEY FROM KEGRKPD WHERE KEGRKPDKEY = '$KEGRKPDKEY' AND UNITKEY = '$UNITKEY' AND KDTAHUN = '$tahun'")->row_array();
                            if(count($cekprog) > 0)
                            {
                                $message ['msg'] = '<script> alert("Data duplicate");</script>';               
                                $this->session->set_flashdata($message);
                                ...
                                (
                                    ...
                                    'IS_RES_GENDER'     =>NULL,
                                    'PAGUTIFDPA'        =>"$PAGUTDPA",
                                    'TARGETSEBELUM'    =>"$TARGETSUBKEGSEBELUM"
                                );
                        ...
                        {
							$tahap = $this->KDTAHAP;
                            $cekprog = $this->db->query("SELECT SUBKEGRKPDKEY FROM SUBKEGRKPD WHERE SUBKEGRKPDKEY = '$SUBKEGRKPDKEY' AND UNITKEY = '$UNITKEY' AND KDTAHUN = '$tahun' AND KDTAHAP = '$tahap'")->row_array();
                            if(count($cekprog) > 0)
                            {
                                $message ['msg'] = '<script> alert("Data duplicate");</script>';               
                                $this->session->set_flashdata($message);
