...
class master extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model(['m_set', 'm_program', 'm_kegiatan', 'm_hspk', 'm_master', 'm_subkegiatan_a']);
...
//*CALL MENU*
	...
	{
		$this->sip->is_menu('0303');
		$this->load->view('master/v_import');
	}
...
//Program
	public function programaster_load($page = 1, $first = FALSE)
	{
		...
		AND MP.UNITKEY in(SELECT  URUSKEY FROM URUSANUNIT U LEFT JOIN DAFTUNIT D ON D.UNITKEY = U.UNITKEY
		...
		$(function() {
			$(document).off('click', blockEntryProgram + '.check-all');
			$(document).on('click', blockEntryProgram + '.check-all', function(e) {
				var checkboxes = $(blockEntryProgram + "input[name='i-check[]']:checkbox");
	...
	public function program_form($act)
	{
		...
		if($act == 'add')
		{
			...
			$dataurusan = $this->db->query("
				SELECT
					URUSKEY,
					(SELECT NMUNIT FROM DAFTUNIT DU WHERE DU.UNITKEY = U.URUSKEY) AS URUSAN
				FROM URUSANUNIT U LEFT JOIN DAFTUNIT D ON D.UNITKEY = U.UNITKEY --ORDER BY NMUNIT ASC
				where  u.UNITKEY = ?",
				[
					$unitkey
				])->result_array();
		...
		elseif($act == 'edit')
		{
			...
			$data = [
				...
				SELECT
					...
					(SELECT NMUNIT FROM DAFTUNIT DU WHERE DU.UNITKEY = U.URUSKEY) AS URUSAN
				FROM URUSANUNIT U LEFT JOIN DAFTUNIT D ON D.UNITKEY = U.UNITKEY --ORDER BY NMUNIT ASC
				where  u.UNITKEY = ? ", $unitkey)->result_array()
			];
	...
	public function program_save($act)
	{
		...
		try
		{
			...
			$uruskey			= $this->input->post('uruskey');
...
//kegiatan
	public function kegiatanmaster_load($page = 1, $first = FALSE)
	{
		...
		$load .= "
			...
			<td><a href='javascript:void(0)' class='btn-kegiatan-master-show-subkegiatan'>{$r['KEGRKPDKEY']}</td>
			<td>{$r['NUKEG']}</td>
			<td>{$r['NMKEG']}</td>
		...
		<script>
			$(function() {
			$(document).off('click', blockEntryKegiatan + '.check-all');
			$(document).on('click', blockEntryKegiatan + '.check-all', function(e) {
				var checkboxes = $(blockEntryKegiatan + "input[name='i-check[]']:checkbox");
				checkboxes.prop('checked', $(this).is(':checked')).not($(this)).change();
			});
		});
		</script>
	...
	// sub kegiatan
	// show
	public function subkegiatanmaster_load($page = 1, $first = FALSE)
	{
		$per_page = 6;
		$kegrkpdkey = $this->input->post('f-kegrkpdkey');
		$search_type = $this->input->post('f-search_type');
		...
		if($search_key)
		{
			switch($search_type)
			{
				case '1' : $search_type = 'MK.SUBKEGRKPDKEY'; break;
		...
		$total = $this->m_subkegiatan_a->getAll($filter, NULL, TRUE)->row_array()['TOTAL_ROW'];
		$rows = $this->m_subkegiatan_a->getAll($filter, [$per_page, $page])->result_array();
		while ($page > 1 AND count($rows) < 1):
		$page--;
		$rows = $this->m_subkegiatan_a->getAll($filter, [$per_page, $page])->result_array();
		endwhile;
		...
		$load .= "
			...
			<td>{$r['SUBKEGRKPDKEY']}</td>
			<td>{$r['NUSUBKEG']}</td>
			<td>{$r['NMSUBKEG']}</td>
			<td class='text-center'><a href='javascript:void(0)' class='btn-entry-sub-kegiatan-form' data-act='edit'><u>Edit</u></a></td>
			<td class='text-center'>
				<div class='checkbox checkbox-inline'>
		...
		$(function() {
			...
			$(blockEntrySubKegiatan + '.block-pagination').html($(blockEntrySubKegiatan + '.pagetemp').html());
		});
		</script>";
		...
		<script>
			$(function() {
			$(document).off('click', blockEntrySubKegiatan + '.check-all');
			$(document).on('click', blockEntrySubKegiatan + '.check-all', function(e) {
				var checkboxes = $(blockEntrySubKegiatan + "input[name='i-check[]']:checkbox");
				checkboxes.prop('checked', $(this).is(':checked')).not($(this)).change();
			});
		});
		</script>
	...
	public function subkegiatan_form($act)
	{
		...
		else
		{
			...
			$this->load->view('master/v_entry_sub_kegiatan', $data);
	...
	public function subkegiatan_save($act)
	{
		...
		try
		{
			...
			$this->form_validation->set_rules('i-nusubkeg', 'Nomor Sub Kegiatan', 'trim|required');
			$this->form_validation->set_rules('i-nmsubkeg', 'Nama Sub Kegiatan', 'trim|required');
			...
			$nusubkeg				= $this->input->post('i-nusubkeg');
			$nmsubkeg				= $this->input->post('i-nmsubkeg');
			$kegrkpdkey				= $this->input->post('i-kegrkpdkey');
			...
			if($act == 'add')
			{
					$newsubkegrkpdkey		= $this->m_set->getNextKey('MSUBKEGRKPD');

				$set = [
					'kegrkpdkey'			=> $kegrkpdkey,
					'subkegrkpdkey'			=> $newsubkegrkpdkey,
					'kdtahun'				=> $this->KDTAHUN,
					'nusubkeg'				=> $nusubkeg,
					'nmsubkeg'				=> $nmsubkeg,
					'type'					=> 'D'
				...
				$affected = $this->m_master->addSubKegiatan($set);
				if($affected !== 1)
				{
					throw new Exception('Sub Kegiatan gagal ditambahkan.', 2);
				}
					$this->m_set->updateNextKey('MSUBKEGRKPD', $newsubkegrkpdkey);
			}
			elseif($act == 'edit')
			{
				$set = [
				...
				$affected = $this->m_master->SubKegiatanUpdate($where, $set);

				if($affected !== 1)
				{
					throw new Exception('Sub Kegiatan gagal Dirubah.', 2);
				}
			}
...
//PAGU
	...
	public function pagu_delete()
	{
		...
		try
		{
			...
			$UNITT = json_encode($unitkey);
			$simpanHistory = $this->m_master->cHistory($nilai, $nmTable, $UNITT);
		}
		catch (Exception $e)
		{
			$this->db->trans_rollback();
