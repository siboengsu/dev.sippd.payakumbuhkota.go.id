...
$(function() {
	...
	$('#renja-matrik-opd-perubahan').submit(function(e) {
		...
		if (idprint=="1") {
				$.post('report_matrik/cetak_matrik_renja_opd_perubahan_skpd/'+id, $(this).serialize(), function(res, status, xhr) {
					if(contype(xhr) == 'json') {
						respond(res);
					} else {
						window.open("report_matrik/cetak_matrik_renja_opd_perubahan_skpd/"+id , "_blank");
					}
					});
							} else if (idprint =="2") {
