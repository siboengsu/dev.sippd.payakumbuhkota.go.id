...
$(function() {
	...
	$(document).on('submit', blockReport + '.renja-matrik-cetak', function(e) {
		...
		e.preventDefault();
		$.post('/c_matrik51perperangkatdaerah/contr_matrik51perperangkatdaerah/'+unitkey+"/"+nomor, $(blockReport + '.renja-matrik-cetak').serializeArray(), function(res, status, xhr) {
			if(contype(xhr) == 'json') {
				respond(res);

			} else {
					console.log(unitkey);
				window.open("c_matrik51perperangkatdaerah/contr_matrik51perperangkatdaerah/"+unitkey+"/"+nomor, "_blank");
			}
		});
