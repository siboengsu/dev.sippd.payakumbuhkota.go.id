	...
	<div class="col-md-12">
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<form class="form-horizontal" id="rekap-opd-cetak">
...
$(function() {
	...
	$('#rekap-opd-cetak').submit(function(e) {
		e.preventDefault();
		var nomor = $('#f-nomor').val();
		var id = $('#reportformat').val();
			if (id=="1") {
			$.post('report_matrik/c_skpd_opd/'+nomor, $(this).serialize(), function(res, status, xhr) {
				if(contype(xhr) == 'json') {
					respond(res);
				} else {
					window.open("report_matrik/c_skpd_opd/"+nomor, "_blank");
				}
			});

		}else if (id=="2") {
			$.post('report/createXLSskpd_opd', $(this).serialize(), function(res, status, xhr) {
				if(contype(xhr) == 'json') {
					respond(res);
				} else {
					window.open("report/createXLSskpd_opd", "_blank");
				}
			});

		}else {
			$.post('report/cetak_skpd_opd_word', $(this).serialize(), function(res, status, xhr) {
				if(contype(xhr) == 'json') {
					respond(res);
				} else {
					window.open("report/cetak_skpd_opd_word/print", "_blank");
				}
			});
		}
		return false;
