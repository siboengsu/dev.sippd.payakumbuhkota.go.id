...
$(function() {
	...
	$('#pagu-opd-cetak').submit(function(e) {
		...
		if (id=="1") {
			$.post('Report_matrik/c_rekapitulasi_pagu_indikatif_nonADK', $(this).serialize(), function(res, status, xhr) {
				if(contype(xhr) == 'json') {
					respond(res);
				} else {
					window.open("Report_matrik/c_rekapitulasi_pagu_indikatif_nonADK/Rekapitulasi_pagu_indikatif_nonADK", "_blank");
				}
					});
		} else if (id=="2") {
