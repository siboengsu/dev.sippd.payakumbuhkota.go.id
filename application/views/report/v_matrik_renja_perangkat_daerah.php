...
<div class="row">
	<div class="col-md-12">
		<div class="page-header">
			<h1>Plafon Anggaran  Matrik Renja Perangkat Daerah</h1>
		</div>
	</div>
</div>
	...
	<div class="col-md-12">
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<form class="form-horizontal" id="plafon-matrik-cetak" name="plafon-matrik-cetak">
...
$(function() {
	...
	$('#plafon-matrik-cetak').submit(function(e) {
		e.preventDefault();
		var id = $('#f-unitkey').val();
		var idprint = $('#reportformat').val();
		if (idprint=="1") {
			$.post('report/matrik_renja_perangkat_daerah_print/'+id, $(this).serialize(), function(res, status, xhr) {
				if(contype(xhr) == 'json') {
					respond(res);
				} else {
					window.open("report/matrik_renja_perangkat_daerah_print/print/"+id, "_blank");
				}
			});
		} else {
			$.post('report/createXLSmatrik_renja_perangkat_daerah/'+id, $(this).serialize(), function(res, status, xhr) {
				if(contype(xhr) == 'json') {
					respond(res);
				} else {
					window.open("report/createXLSmatrik_renja_perangkat_daerah/"+id);
				}
			});
		}
		return false;
