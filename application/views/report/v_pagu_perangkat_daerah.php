...
<div class="row">
	<div class="col-md-12">
		<div class="page-header">
			<h1>Plafon Anggaran Pagu Perangkat Daerah</h1>
		</div>
	</div>
</div>
...
$(function() {
	...
	$('#plafon-pagu-opd-cetak').submit(function(e) {
		...
		var id = $('#reportformat').val();
				if (id=="1") {
				$.post('report/pagu_perangkat_daerah_print', $(this).serialize(), function(res, status, xhr) {
					if(contype(xhr) == 'json') {
						respond(res);
					} else {
						window.open("report/pagu_perangkat_daerah_print/print", "_blank");
					}
				});
			} else {
			$.post('report/createXLSpagu_perangkatdaerah', $(this).serialize(), function(res, status, xhr) {
				if(contype(xhr) == 'json') {
					respond(res);
				} else {
					window.open("report/createXLSpagu_perangkatdaerah", "_blank");
				}
			});
		}
