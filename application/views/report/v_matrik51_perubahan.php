	...
	<div class="col-md-12">
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<form class="form-horizontal" id="matrik51_cetak_perubahan">
...
$(function() {
	...
	$('#matrik51_cetak_perubahan').submit(function(e) {
		e.preventDefault();
			var nomor = $('#f-nomor').val();
			var id = $('#reportformat').val();
		if (id=="1") {

		$.post('c_lap_matrik51_perubahan/contr_matrik51_perubahan/'+nomor, $(this).serialize(), function(res, status, xhr) {
			if(contype(xhr) == 'json') {
				respond(res);
			} else {
				window.open("c_lap_matrik51_perubahan/contr_matrik51_perubahan/"+nomor, "_blank");
			}
		});
		} else if (id=="2") {
		$.post('report/createXLSmatrik_51_perubahan', $(this).serialize(), function(res, status, xhr) {
			if(contype(xhr) == 'json') {
				respond(res);
			} else {
				window.open("report/createXLSmatrik_51_perubahan", "_blank");
			}
		});

		} else {
		$.post('report/matrik51_print_word_perubahan', $(this).serialize(), function(res, status, xhr) {
			if(contype(xhr) == 'json') {
				respond(res);
			} else {
				window.open("report/matrik51_print_word_perubahan/print", "_blank");
			}
		});
	}
		return false;
