...
class libs_matrik51perubahanperopd extends FPDF
{
    public $tablewidths;
    public $footerset;
    public $headerset;
    ...
    public function setTotalPlusA($input10){
        $this->total_pagu_plus_a = $input10;
    }
    ...
    public function Header()
    {
        ...
        if (!isset($this->headerset[$this->page])) {
            ...
            $this->Image(base_url('assets/img/logo-kota-payakumbuh.png'), 5, 6, 10);
            // Arial bold 15
            $this->SetFont('Arial', 'B', 6);
            // Move to the right
            $this->Cell(225);
            // Title
            $this->MultiCell(63, 4, 'Rencana Kerja Perubahan Tahun ' .$this->data , 0, 'R');
            $this->Ln(2);
            // Line break
            ...
            if ($this->PageNo()==1) {
                // mencetak string
                $this->Cell(285, 4, 'Tabel III.I ', 0, 1, 'C');
            ...
            $this->SetFont('Arial','B',7);
              ...
              $this->Cell($J+$K+$L,5,"Pagu Indikatif Tahun ",1,0,'C');
              ...
              $this->MultiCell($F+$G+$N,3,'Indikator Kegiatan (outcome) / Indikator Subkegiatan (output)','L,R','C');
              $this->Ln(0);
              ...
              $this->MultiCell($P,5.5,' Pagu 2022','L,R','C');
              $this->Ln(0);
              $this->SetXY($x +$A+$B+$C+$D+$E+$F+$G+$N+$H+$I+$O+$J+$K+$L+$P, $tinggi);
              $this->MultiCell($M,6,'Jenis Kegiatan','L,R','C');
              $this->Ln(0);
              $this->Cell(0,5,'',0,1);
              //line 3
              $this->Cell($A,4,'',0,0);
    ...
    public function morepagestable($datas, $lineheight=4)
    {
        ...
        $this->SetFont('Arial','B',7);
        $this->Cell(193,8,'Total',1,0,'C');
        $this->Cell(21,8,"$this->total_pagu",1,0,'R');
        $this->Cell(21,8,"$this->total_pagu_plus",1,0,'R');
        $this->Cell(21,8,"$this->total_pagu_sisa",1,0,'R');
        $this->Cell(21,8,"$this->total_pagu_plus_a",1,0,'R');
        ...
        $this->page = $maxpage;
        $this->SetXY(277, $h, $fullwidth+$l, $h);
    }
}
