...
class libs_matrikrpjmd extends FPDF
{
    public $tablewidths;
    public $footerset;
    public $headerset;
    ...
    public function Header()
    {
        if (!isset($this->headerset[$this->page])) {
            ...
            if ($this->PageNo()==1) {
                ...
                $this->Cell(285, 4, "REKAPITULASI RENCANA PROGRAM DAN KEGIATAN PRIORITAS DAERAH TAHUN ANGGARAN $this->data ", 0, 1, 'C');
            ...
            $this->Cell($C, 6, '', 'T', 0);
            $this->Cell($D+$E+$F+$G+$H+$I, 5, 'Indikasi Kinerja Program/Kegiatan/Sub Kegiatan', 1, 0, 'C');
            $this->Cell($J, 5, '', 'T,R', 0, 'C');
            $this->Cell($K, 5, '', 'T,R', 0, 'C');
            $this->Cell($L, 5, '', 'T,R', 0, 'C');

            $this->Cell(0, 4, '', 0, 1);
            //line 2
            $this->Cell($A, 4, '', 0, 0);
            $this->MultiCell($B, 3, 'Urusan Pemerintah Daerah/Program/Kegiatan/
			Sub Kegiatan', 0, 'C');
            $this->SetXY($x +$A+$B, $tinggi);
            $this->MultiCell($C, 3, 'Prioritas/RG', 'L,R', 'C');
            $this->SetXY($x +$A+$B+$C, $tinggi+2);
            $this->MultiCell($D+$E, 6, 'IKD / IKU Perangkat Daerah', 'L,R', 'C');
            $this->Ln(0);
            $this->SetXY($x +$A+$B+$C+$D+$E, $tinggi+2);
            $this->MultiCell($F+$G, 3, '                   Indikator Kegiatan (outcome)/                    Indikator Subkegiatan (output)', 'L,R', 'C');
            $this->Ln(0);
            $this->SetXY($x +$A+$B+$C+$D+$E+$F+$G, $tinggi+2);
            $this->MultiCell($H+$I, 6, 'Indikator Program', 'L,R', 'C');
            $this->Ln(0);
            $this->SetXY($x  +$A+$B+$C+$D+$E+$F+$G+$H+$I, $tinggi+2);
            $this->MultiCell($J, 4.5, "Dana Indikatif Tahun $this->data ", 'L,R', 'C');
            $this->Ln(0);
            $this->SetXY($x +$A+$B+$C+$D+$E+$F+$G+$H+$I +$J, $tinggi+2);
            $this->MultiCell($K, 4.5, "Prakiraan Maju Tahun $this->dataplus", 'L,R', 'C');
            $this->Ln(0);
            $this->SetXY($x +$A+$B+$C+$D+$E+$F+$G+$H+$I +$J+$K, $tinggi+2);
            $this->MultiCell($L, 4.5, 'Penanggung Jawab', 'L,R', 'C');
            $this->Ln(0);
    ...
    public function morepagestable($datas, $lineheight=4)
    {
        ...
        $this->SetFont('Arial','B',7);
        $this->Cell(216,5,'Total',1,0,'C');
        $this->Cell(22,5,"$this->total_pagu",1,0,'R');
        $this->Cell(22,5,"$this->total_pagu_plus",1,0,'R');
